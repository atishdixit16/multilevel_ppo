...
def eval_actions(env, actions_, base_steps=0):
    ...
    while not done:
        ...
        states.append(env.state['s'].copy())
        actions.append(env.env_action.copy())
        i=i+1
    return states, actions, rewards 

def eval_env_time(env, actions_, base_steps=0):
    obs, done = env.reset(), False
    ts = []
    i=0
    while not done:
...
def eval_frac_flow(env, actions_, x_loc, y_loc):
    ...
    while not done:
        ...
        for x,y in zip(x_loc, y_loc):
            ff_values.append(env.ressim_params.f_fn(env.state['s'][x,y]))
        i=i+1
