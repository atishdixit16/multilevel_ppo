...
def get_ff_coords(k_samples, env, x_loc, y_loc):
    ...
    actions_base = np.array([ np.append(env.ressim_params.q[env.ressim_params.q>1e-5], -env.ressim_params.q[env.ressim_params.q<-1e-5]) for _ in range(env.ressim_params.terminal_step) ])
    actions_base = actions_base.reshape(env.ressim_params.terminal_step, -1)
