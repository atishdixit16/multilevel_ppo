...
def get_min_tr_data(case_dir, seeds, result_type='train', episode_len=1):
    log_dirs=[]
    for seed in seeds:
        log_dir = case_dir+'/seed_'+str(seed)+'/results_'+result_type
        log_dirs.append(log_dir)
    ...
    rs = np.array(rs).reshape(len(seeds),-1)
    t_data = np.array(ts[0])/episode_len
    r_data = np.min(rs, axis=0)
    return t_data, r_data

def get_max_tr_data(case_dir, seeds, result_type='train', episode_len=1):
    log_dirs=[]
    for seed in seeds:
        log_dir = case_dir+'/seed_'+str(seed)+'/results_'+result_type
        log_dirs.append(log_dir)
...
def get_sar_data(envs, rl_k_eval_indices, opt_dir, models, model_names, model_base_steps = 0):
    ...
    base_actions = np.ones((envs[0].ressim_params.terminal_step,envs[0].action_space.shape[0]))
    terminal_steps = envs[0].ressim_params.terminal_step
