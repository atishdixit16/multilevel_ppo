...
#     plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9)
    for i,(ax, state, action) in enumerate(zip(axs, states, actions)):
        ...
        if show_wells:
            ...
            ax.set_xlim([-marker_size_ref, states[0].shape[1]+marker_size_ref])
            ax.set_ylim([-marker_size_ref, states[0].shape[0]+marker_size_ref])
        ax.set_title(f'$r_{i+1}={reward:.2}$')
#         fig.canvas.draw()
    return fig
...
def plot_actions(r_array, a_array, s_array, case_names, 
    ...
    for i,(state_array, action_array, reward_array, title) in enumerate( zip(s_array, a_array, r_array, case_names) ):
        for j,( state, action) in enumerate(zip(state_array, action_array)):
            ...
            if j==0:
                axs[i,j].text(-45,state.shape[1]/2,title)
...
def plot_rewards(r_array_eval, axs, model_names):
    ...
    axs.legend(model_names+['DE', 'No policy'])
    axs.set_xlabel('permeability sample index')
    axs.set_ylabel('sweep efficiency')
